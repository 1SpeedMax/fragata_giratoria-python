...
def login_personalizado(request):
    if request.user.is_authenticated:
        if request.user.rol and request.user.rol.nombre_rol:
            rol = request.user.rol.nombre_rol.upper().strip()
            if rol == 'ADMIN':
    ...
    if request.method == 'POST':
        ...
        if usuario:
            estado_usuario = str(getattr(usuario, 'estado', '') or '').upper().strip()
            if estado_usuario == 'SUSPENDIDO':
                messages.error(request, "Cuenta SUSPENDIDA. Contacta con el equipo.")
                return render(request, 'home/login.html')
            if estado_usuario == 'INACTIVO':
                messages.error(request, "Cuenta DESACTIVADA. Contacta con el equipo.")
                return render(request, 'home/login.html')

            if getattr(usuario, 'rol', None):
                estado_rol = str(getattr(usuario.rol, 'estado', '') or '').upper().strip()
                if estado_rol == 'SUSPENDIDO':
                    messages.error(request, "El rol está suspendido. Contacta al administrador.")
                    return render(request, 'home/login.html')
                if estado_rol == 'INACTIVO':
                    messages.error(request, "El rol está inactivo. Contacta soporte.")
                    return render(request, 'home/login.html')

        user = authenticate(request, username=email, password=password)
        if user is None and usuario:
            user = authenticate(request, username=usuario.nombre_usuario, password=password)
        ...
        if user is not None:
            ...
            messages.warning(request, "Tu rol no está configurado correctamente.")
            return redirect('inicio')
        else:
...
def es_admin(user):
    return (
        ...
        user.rol and
        user.rol.nombre_rol.upper().strip() == "ADMIN"
    )
...
def es_mesero(user):
    return (
        user.is_authenticated and
        user.rol and
        user.rol.nombre_rol.upper().strip() == "MESERO"
    )
...
def es_cocinero(user):
    return (
        user.is_authenticated and
        user.rol and
        user.rol.nombre_rol.upper().strip() == "COCINERO"
    )
...
def es_cliente(user):
    return (
        user.is_authenticated and
        user.rol and
        user.rol.nombre_rol.upper().strip() == "CLIENTE"
    )
